...
if ! curl -fsS "${API_URL}/health" >/dev/null 2>&1; then
  (
    cd "$ROOT_DIR/server"
    TECHNICAL_CREDENTIALS_KEY="$TECHNICAL_CREDENTIALS_KEY" sbt run
...
session_count_before="$(
  api_json GET "$ADMIN_TOKEN" "/admin/sessions" |
    jq --arg database_id "$DATABASE_ID" '[.[] | select(.database.id == $database_id)] | length'
)"
...
if (( session_count_after <= session_count_before )); then
  echo "Expected a new admin session record after successful proxy login" >&2
  exit 1
fi
