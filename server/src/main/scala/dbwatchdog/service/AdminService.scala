        ...
        db.transact(
          for {
            ...
            users <-
              ...
              else repos.users.findByIds(sessions.map(_.userId).toSet)
            teams <-
              if users.isEmpty then List.empty[Team].pure[ConnectionIO]
              else repos.teams.findByIds(users.map(_.teamId).toSet)
            databases <-
